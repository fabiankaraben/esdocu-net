...
Los ayudantes de color y fondo combinan el poder de nuestras [`.text-*` (utilidades)](/docs/5.3/utilities/colors/) y [`.bg-*` (utilidades)](/docs/5.3/utilities/background/) en una clase. Usando nuestra función Sass `color-contrast()`, determinamos automáticamente un `color` de contraste para un `background-color` particular.
...
Úsalos en lugar de las clases combinadas `.text-*` y `.bg-*` , como en [insignias](/docs/5.3/components/badge/#background-colors):
...
O en [tarjetas](/docs/5.3/components/card/#background-and-color):
