---
title: Opciones · Bootstrap en Español v5.3
...
| --- | --- | --- |
| `$spacer` | `1rem` (predeterminado), o cualquier valor > 0 | Especifica el valor de espaciado predeterminado para generar mediante programación nuestras [utilidades de espaciado](/docs/5.3/utilities/spacing/). |
| `$enable-dark-mode` | `true` (predeterminado) o `false` | Habilita el [soporte para el modo oscuro](/docs/5.3/customize/color-modes/#dark-mode) integrado en todo el proyecto y sus componentes. |
...
| `$enable-reduced-motion` | `true` (predeterminado) o `false` | Habilita la [`prefers-reduced-motion` media query](/docs/5.3/getting-started/accessibility/#reduced-motion), que suprime ciertas animaciones/transiciones basadas en las preferencias del navegador/sistema operativo de los usuarios. |
...
| `$enable-rfs` | `true` (predeterminado) o `false` | Habilita globalmente [RFS](/docs/5.3/getting-started/rfs/). |
...
| `$enable-negative-margins` | `true` o `false` (predeterminado) | Permite la generación de [utilidades de margen negativo](/docs/5.3/utilities/spacing/#negative-margin). |
...
| `$enable-smooth-scroll` | `true` (predeterminado) o `false` | Aplica `scroll-behavior: smooth` globalmente, excepto para los usuarios que solicitan movimiento reducido a través de [`prefers-reduced-motion` media query](/docs/5.3/getting-started/accessibility/#reduced-motion) |