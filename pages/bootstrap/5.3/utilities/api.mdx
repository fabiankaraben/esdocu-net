...
| `rfs` | Opcional | `false` | Booleano para habilitar el [rescalado fluid con RFS](/docs/5.3/getting-started/rfs/). |
...
Como una lista, como con las [`text-decoration` (utilidades)](/docs/5.3/utilities/text/#text-decoration):

    ```scss filename="SCSS"
    values: none underline line-through
    ```

Como mapa, como con las [`opacity` (utilidades)](/docs/5.3/utilities/opacity/):
...
Como una variable Sass que establece la lista o mapa, como en nuestra [`position` (utilidades)](/docs/5.3/utilities/position/):
...
Agrega el booleano `responsive` para generar utilidades responsive (por ejemplo, `.opacity-md-25`) en [todos los puntos de interrupción](/docs/5.3/layout/breakpoints/).
