...
Cambia la opacidad alfa del valor de color del enlace `rgba()` con utilidades. Ten en cuenta que los cambios en la opacidad de un color pueden generar enlaces con [_insuficiente_ contraste](/docs/5.3/getting-started/accessibility/#color-contrast).
...
[Los ayudantes de enlaces de colores](/docs/5.3/helpers/colored-links/) se han actualizado para combinarlos con nuestras utilidades de enlaces. Utiliza las nuevas utilidades para modificar la opacidad del enlace, la opacidad del subrayado y el desplazamiento del subrayado.
...
Además de las siguientes funciones de Sass, considera leer sobre nuestras [propiedades personalizadas de CSS](/docs/5.3/customize/css-variables/) incluidas (también conocidas como variables CSS) para colores y más.

### API de utilidades de Sass [#sass-utilities-api]

Las utilidades de enlace se declaran en nuestra API de utilidades en `scss/_utilities.scss`. [Aprende a utilizar la API de utilidades.](/docs/5.3/utilities/api/#using-the-api)
