...
Mostrar clases de utilidad que se aplican a todos los [puntos de interrupción](/docs/5.3/layout/breakpoints/), desde `xs` a `xxl`, no tienen abreviaturas de punto de interrupción. Esto se debe a que esas clases se aplican desde `min-width: 0;` en adelante y, por lo tanto, no están vinculadas a una media query. Los puntos de interrupción restantes, sin embargo, incluyen una abreviatura de punto de interrupción.
...
Las utilidades de visualización se declaran en nuestra API de utilidades en `scss/_utilities.scss`. [Aprende a utilizar la API de utilidades.](/docs/5.3/utilities/api/#using-the-api)
