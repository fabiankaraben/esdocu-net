...
* Usa nuestro [espaciado](/docs/5.3/utilities/spacing/) y clases [flex](/docs/5.3/utilities/flex/) de utilidad para controlar el espaciado y la alineación dentro de las barras de navegación.
* Las barras de navegación responden de manera predeterminada, pero puedes modificarlas fácilmente para cambiar eso. El comportamiento responsive depende de nuestro complemento Collapse [JavaScript](https://esdocu.dev/javascript).
...
Este ejemplo usa las clases de utilidad [fondo](/docs/5.3/utilities/background/) (`bg-body-tertiary`) y [espaciado](/docs/5.3/utilities/spacing/) (`me-auto`, `mb-2`, `mb-lg-0`, `me-2`).
...
Los elementos secundarios inmediatos de `.navbar` usan diseño flex y por defecto serán `justify-content: space-between`. Utiliza [utilidades flex](/docs/5.3/utilities/flex/) adicionales según sea necesario para ajustar este comportamiento.
...
**Nuevas barras de navegación oscuras en v5.3.0 —** Hemos dejado de usar `.navbar-dark` en favor del nuevo `data-bs-theme="dark"`. Agrega `data-bs-theme="dark"` al `.navbar` para habilitar un modo de color específico del componente. [Obtén más información sobre nuestros modos de color.](/docs/5.3/customize/color-modes/)
...
Utiliza nuestras [utilidades de posición](/docs/5.3/utilities/position/) para colocar barras de navegación en posiciones no estáticas. Elige entre fijo en la parte superior, fijo en la parte inferior, pegado en la parte superior (se desplaza con la página hasta que llega a la parte superior y luego permanece allí) o pegado en la parte inferior (se desplaza con la página hasta que llega al final y luego permanece allí).
