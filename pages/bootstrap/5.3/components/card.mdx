...
Utiliza nuestras [utilidades de tamaño disponibles](/docs/5.3/utilities/sizing/) para configurar rápidamente el ancho de una tarjeta.
...
Puedes cambiar rápidamente la alineación del texto de cualquier tarjeta, en su totalidad o en partes específicas, con nuestras [clases de alineación de texto](/docs/5.3/utilities/text/#text-alignment).
...
Agrega algo de navegación al encabezado (o bloque) de una tarjeta con los [componentes de navegación](/docs/5.3/components/navs-tabs/) de Bootstrap.
...
Establece un `background-color` con un `color` de primer plano contrastante con [nuestros `.text-bg-{color}` ayudantes](/docs/5.3/helpers/color-background/). Anteriormente era necesario emparejar manualmente tu elección de utilidades para diseñar [`.text-{color}`](/docs/5.3/utilities/colors/) y [`.bg-{color}`](/docs/5.3/utilities/background/), que aún puedes usar si lo prefieres.
...
Usa [utilidades de borde](/docs/5.3/utilities/borders/) para cambiar solo el `border-color` de una tarjeta. Ten en cuenta que puedes colocar clases `.text-{color}` en la `.card` principal o en un subconjunto del contenido de la tarjeta como se muestra a continuación.
