...
El complemento de colapso admite el colapso horizontal. Agrea la clase modificadora `.collapse-horizontal` para realizar la transición del `width` en lugar de `height` y establece un `width` en el elemento hijo inmediato. Siéntete libre de escribir tu propio Sass personalizado, usar estilos en línea o usar nuestras [utilidades de ancho](/docs/5.3/utilities/sizing/).
...
Ten en cuenta que la implementación actual de Bootstrap no cubre las diversas interacciones de teclado _opcionales_ descritas en [Patrón de acordeón de la Guía de prácticas de creación de ARIA](https://www.w3.org/WAI/ARIA/apg/patterns/accordion/), deberás incluirlos tú mismo con [JavaScript](https://esdocu.dev/javascript) personalizado.
...
Para agregar administración de grupos tipo acordeón a un área plegable, agrega el atributo de datos `data-bs-parent="#selector"`. Consulta la [página del acordeón](/docs/5.3/components/accordion/) para obtener más información.
