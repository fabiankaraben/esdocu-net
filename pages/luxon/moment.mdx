...
See the [formatting guide](/luxon/formatting) for more about the string-outputting methods.
...
1.  Luxon durations have more sophisticated conversion capabilities. They can convert from one set of units to another using `shiftTo`. They can also be configured to use different unit conversions. See [Duration Math](/luxon/math#duration-math) for more.
