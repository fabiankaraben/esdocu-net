...
If you specify a zone and your environment doesn't support that zone, you'll get an [invalid](/luxon/validity) DateTime. That could be because the environment doesn't support zones at all (generally browsers older than Luxon supports), because for whatever reason it doesn't support that _particular_ zone, or because the zone is just bogus. Like this:
